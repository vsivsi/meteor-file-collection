...
To cache files in the browser use the '?cache=172800' query in the request URL, where 172800 (48h) is the time in seconds. This wil set the header response information to 'cache-control:max-age=172800, private'. Caching is useful when streaming videos or audio files to avoid unwanted calls to the server.
